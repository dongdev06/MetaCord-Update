...
module.exports = function (_defaultFuncs, api, _ctx) {
    ...
    try {
        ...
        var FindUID = require('../utils/Extension');
        if (Link.includes('facebook.com') || Link.includes('Facebook.com') || Link.includes('fb')) {
            ...
            if (LinkSplit.indexOf("https:") == 0) {
              if (!isNaN(LinkSplit[3]) && !Link.split('=')[1]  && !isNaN(Link.split('=')[1])) {
                api.sendMessage('Wrong link, link should be formatted as follows: facebook.com/Lazic.Kanzu',global.Fca.Data.event.threadID,global.Fca.Data.event.messageID);
            ...
            callback(null, null);
            api.sendMessage('Wrong link, link needs to be Facebook',global.Fca.Data.event.threadID,global.Fca.Data.event.messageID);
        }
    }
    catch (e) {
